...
function EditorPageInner() {
  ...
  const handleGenerate = useCallback(async (formats: string[]) => {
    ...
    await Promise.all(formats.map(async(fmt) => {
      ...
      return fetch("/api/pieces",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({campaignId:cid,name:label,format:fmt,data:scaled,upsert:true})});
    }));
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#F7F7F7",fontFamily:"DM Sans, sans-serif"}}>
      ...
      <div style={{height:"48px",background:"#FFF",borderBottom:"1px solid #E5E5E5",display:"flex",alignItems:"center",padding:"0 16px",gap:"12px",zIndex:10,flexShrink:0}}>
        ...
        {isPiece && (
          <button onClick={()=>{ window.location.href="/pieces?campaignId="+(activeCampaignId??"")+"&export="+pieceId; }} style={{padding:"6px 16px",border:"none",borderRadius:"8px",background:"#E45804",color:"#FFF",fontSize:"0.8rem",fontWeight:700,cursor:"pointer"}}>
            ⬆ Exportar
          </button>
        )}
            ...
            <div style={{padding:"12px",borderBottom:"1px solid #E5E5E5"}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                ...
                <div style={{display:"flex",gap:"8px"}}>
                  ...
                  <div style={{flex:1}}>
                    <label style={{fontSize:"0.72rem",color:"#888",display:"block",marginBottom:"3px"}}>Entrelinha</label>
                    <input type="number" value={textProps.lineHeight??1.16} min={0.5} max={5} step={0.05}
                      onChange={e=>updateTextProp("lineHeight",Number(e.target.value))}
                      style={{width:"100%",padding:"5px 8px",border:"1px solid #E5E5E5",borderRadius:"6px",fontSize:"0.8rem",boxSizing:"border-box"}}/>
                  </div>
                </div>
