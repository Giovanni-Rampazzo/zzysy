...
function scaleJsonToFormat(json: any, origW: number, origH: number, newW: number, newH: number) {
  ...
  scaled.objects = (scaled.objects??[]).map((obj:any)=>({...obj,left:(obj.left??0)*factor+offsetX,top:(obj.top??0)*factor+offsetY,scaleX:(obj.scaleX??1)*factor,scaleY:(obj.scaleY??1)*factor,fontSize:obj.fontSize?Math.round(obj.fontSize*factor):undefined}));
  return scaled;
}
...
// ─── EXPORT DIALOG ───────────────────────────────────────────────
function ExportDialog({ onClose, onGenerate, generating }: { onClose:()=>void; onGenerate:(formats:string[])=>void; generating:boolean }) {
...
function EditorPageInner() {
  ...
  const applyHistory = useCallback((json:string)=>{
    ...
    canvas.loadFromJSON(JSON.parse(json),()=>{
      ...
      setLayers(objs.map((o,i)=>{if(!o.layerId)o.layerId="loaded-"+i;const t=(o.type??"").toLowerCase();return {id:o.layerId,type:(t==="i-text"?"text":t) as LayerType,name:o.text?o.text.substring(0,20):t==="rect"?"Retângulo":t==="circle"?"Círculo":"Imagem",visible:o.visible!==false,locked:!!o.lockMovementX};}));
  ...
  const updateTextProp = (prop:string,value:any)=>{ const canvas=fabricRef.current; if (!canvas) return; const obj=canvas.getActiveObject() as any; if (!obj) return; obj.set({[prop]:value}); canvas.renderAll(); setTextProps(prev=>({...prev,[prop]:value})); setIsDirty(true); };
  const updateShapeColor = (color:string)=>{ const canvas=fabricRef.current; if (!canvas) return; const obj=canvas.getActiveObject() as any; if (!obj) return; obj.set({fill:color}); canvas.renderAll(); setShapeColor(color); setIsDirty(true); };
  ...
  useEffect(()=>{
    ...
    fetch("/api/campaigns").then(r=>r.json()).then(list=>{const c=Array.isArray(list)?list.find((x:any)=>x.id===campaignId):null;if(c)setCampaignName(c.name);});
    fetch(`/api/campaigns/${campaignId}/matrix`).then(r=>r.ok?r.json():null).then(res=>{
      const data=res?.data; if (!data) return;
      canvas.loadFromJSON(data,()=>{canvas.requestRenderAll();setTimeout(()=>{canvas.requestRenderAll();loadLayers(canvas);setIsDirty(false);historyRef.current=[JSON.stringify(data)];historyIndexRef.current=0;syncHistory();},150);});
    });
  ...
  useEffect(()=>{ const canvas=fabricRef.current; if (!canvas) return; canvas.setZoom(zoom); canvas.setDimensions({width:canvasSize.w*zoom,height:canvasSize.h*zoom}); canvas.renderAll(); },[zoom,canvasSize]);
  ...
  const addImage = useCallback(async(file:File)=>{
    ...
    img.set({left:100,top:100}); (img as any).layerId=id; canvas.add(img); canvas.setActiveObject(img); canvas.renderAll();
  ...
  const toggleVisible = useCallback((id:string)=>{ const canvas=fabricRef.current; if (!canvas) return; canvas.getObjects().forEach((o:any)=>{if(o.layerId===id)o.visible=!o.visible;}); canvas.renderAll(); setLayers(prev=>prev.map(l=>l.id===id?{...l,visible:!l.visible}:l)); },[]);
  const toggleLock = useCallback((id:string)=>{ const canvas=fabricRef.current; if (!canvas) return; canvas.getObjects().forEach((o:any)=>{if(o.layerId===id){const lock=!o.lockMovementX;o.set({lockMovementX:lock,lockMovementY:lock,lockScalingX:lock,lockScalingY:lock,selectable:!lock});}}); canvas.renderAll(); setLayers(prev=>prev.map(l=>l.id===id?{...l,locked:!l.locked}:l)); },[]);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#F7F7F7",fontFamily:"DM Sans, sans-serif"}}>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        ...
        {/* PAINEL DIREITO */}
        <aside style={{width:"240px",background:"#FFF",borderLeft:"1px solid #E5E5E5",display:"flex",flexDirection:"column",flexShrink:0}}>
          {isText&&(
          ...
          <div style={{flex:1,overflow:"auto",padding:"6px"}}>
            ...
            {[...layers].reverse().map(layer=>(
              <LayerItem key={layer.id} layer={layer} selected={selectedId===layer.id}
                onSelect={()=>{setSelectedId(layer.id);const canvas=fabricRef.current;if(!canvas)return;const obj=canvas.getObjects().find((o:any)=>o.layerId===layer.id);if(obj){canvas.setActiveObject(obj);canvas.renderAll();}}}
                onToggleVisible={()=>toggleVisible(layer.id)} onToggleLock={()=>toggleLock(layer.id)}
              />
            ))}
          </div>
        </aside>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}}>
          <div style={{background:"#FFF",borderRadius:"12px",padding:"28px 32px",width:"340px",boxShadow:"0 8px 32px rgba(0,0,0,0.18)"}}>
            ...
            <div style={{display:"flex",gap:"8px",justifyContent:"flex-end"}}>
              <button onClick={()=>{window.location.href="/campaigns";}} style={{padding:"7px 16px",border:"1px solid #E5E5E5",borderRadius:"8px",background:"#FFF",color:"#555",fontSize:"0.85rem",fontWeight:600,cursor:"pointer"}}>Não salvar</button>
              <button onClick={()=>{save().then(()=>{window.location.href="/campaigns";});}} style={{padding:"7px 16px",border:"none",borderRadius:"8px",background:"#111",color:"#FFF",fontSize:"0.85rem",fontWeight:700,cursor:"pointer"}}>Salvar e fechar</button>
            </div>
          </div>
        </div>
      )}
