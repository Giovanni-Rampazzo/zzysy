...
function PiecesPageInner() {
  ...
  async function duplicatePiece(piece: Piece) {
    const res = await fetch("/api/pieces",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({campaignId:piece.campaign.id,name:piece.name+" (copia)",format:piece.format,data:{}})});
  ...
  return (
    <div style={{ display:"flex",height:"100vh",background:colors.background,fontFamily:"Inter, sans-serif" }}>
