...
function ExportsPageInner() {
  ...
  const deleteSelected = () => {
    if (!confirm("Deletar "+selected.length+" entrega(s)?")) return;
    Promise.all(selected.map((id:string) => fetch("/api/deliveries/"+id, {method:"DELETE"}))).then(()=>{
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"20px"}}>
            ...
            <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
              ...
              <button onClick={()=>{ if(selected.length>0){ deleteSelected(); } else { setSelectMode(true); } }}
                ...
                {"🗑 Apagar"+(selected.length>0?" "+selected.length:"")}
              </button>
