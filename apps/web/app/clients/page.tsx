    ...
    <div style={{display:"flex",height:"100vh",background:"#FFF",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{marginLeft:"var(--sidebar-w,220px)",flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{padding:"28px 40px 0",borderBottom:b,flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"20px"}}>
            ...
            <button onClick={openCreate} style={{padding:"8px 20px",background:P,color:"#FFF",border:"none",borderRadius:"8px",fontSize:"0.875rem",fontWeight:700,cursor:"pointer",height:"36px",fontFamily:"'DM Sans',sans-serif"}}>+ Novo cliente</button>
          </div>
        ...
        <div style={{flex:1,overflow:"auto",padding:"24px 40px"}}>
          {loading ? <p style={{color:"#888"}}>Carregando...</p> : filtered.length===0 ? (
            <div style={{textAlign:"center",padding:"80px 0",color:"#888"}}>
              ...
              <p style={{fontWeight:600,color:"#111",margin:"0 0 24px"}}>Nenhum cliente cadastrado</p>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:"16px"}}>
              {filtered.map(c=>(
                <div key={c.id} style={{border:b,borderRadius:"12px",padding:"20px",background:"#FFF",cursor:"pointer"}}
                  ...
                  <div style={{display:"flex",gap:"8px",marginTop:"12px"}}>
                    <button onClick={()=>router.push("/clients/"+c.id)} style={{flex:1,padding:"7px 0",background:P,color:"#FFF",border:"none",borderRadius:"8px",fontSize:"0.8rem",fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Ver campanhas</button>
                    <button onClick={()=>openEdit(c)} style={{flex:1,padding:"7px 0",background:"transparent",color:"#888",border:b,borderRadius:"8px",fontSize:"0.8rem",fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Editar</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {showModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}}>
          <div style={{background:"#FFF",borderRadius:"16px",padding:"32px",width:"560px",maxWidth:"90vw",maxHeight:"85vh",overflow:"auto",fontFamily:"'DM Sans',sans-serif"}}>
            ...
            <div style={{marginBottom:"12px"}}><label style={lbl}>Endereço</label><input value={editing.address||""} onChange={e=>set("address",e.target.value)} style={inp} placeholder="Rua, número"/></div>
            ...
            <div style={{display:"flex",gap:"8px",justifyContent:"flex-end"}}>
              <button onClick={()=>setShowModal(false)} style={{padding:"8px 20px",background:"transparent",color:"#888",border:b,borderRadius:"8px",fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>Cancelar</button>
              <button onClick={save} disabled={saving} style={{padding:"8px 24px",background:P,color:"#FFF",border:"none",borderRadius:"8px",fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{saving?"Salvando...":editing.id?"Salvar":"Criar"}</button>
            </div>
          </div>
        </div>
      )}
