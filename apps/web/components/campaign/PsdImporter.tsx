...
export function PsdImporter({ campaignId, onImported }: Props) {
    ...
    try {
      ...
      for (const layer of allLayers) {
        ...
        if (layer.text) {
          ...
          try { imageUrl = (layer.canvas as HTMLCanvasElement).toDataURL("image/png") } catch {}
          assets.push({ label: name, type: "IMAGE", imageUrl, posX: left, posY: top, width, height, zIndex })
        }
        zIndex--
      }
      if (assets.length === 0) { setError("Nenhum layer extraido."); return }
  ...
  async function confirmImport() {
    ...
    try {
      ...
      onImported()
    } catch (e: any) { setError(e?.message ?? "Erro") } finally { setLoading(false); setPending(null) }
  }
  ...
  return (
    <>
      ...
      <button
        style={{ background: "#1a1a1a", border: "1px solid #333", color: "#aaa", padding: "8px 16px", borderRadius: 6, fontSize: 13, cursor: "pointer" }}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#1a1a1a", border: "1px solid #333", borderRadius: 12, padding: 32, maxWidth: 480, width: "90%" }}>
            ...
            <div style={{ fontSize: 13, color: "#888", marginBottom: 8 }}>
              <strong style={{ color: "#fff" }}>{pending.assets.length} layers</strong> — {pending.canvasWidth}x{pending.canvasHeight}px
            </div>
            <div style={{ fontSize: 11, color: "#888", marginBottom: 16, maxHeight: 160, overflowY: "auto", background: "#111", borderRadius: 6, padding: 8 }}>
              {pending.assets.map((a: any, i: number) => (
                <div key={i} style={{ padding: "3px 0", borderBottom: "1px solid #222" }}>
                  {a.type === "TEXT" ? "T" : "IMG"} <strong style={{ color: "#fff" }}>{a.label}</strong>
            ...
            <div style={{ fontSize: 12, color: "#f87171", marginBottom: 24, padding: "8px 12px", background: "rgba(248,113,113,0.1)", borderRadius: 6 }}>
              Assets existentes serao substituidos.
            </div>
