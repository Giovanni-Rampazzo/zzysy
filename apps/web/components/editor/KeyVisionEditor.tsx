...
export function KeyVisionEditor({ campaignId }: { campaignId: string }) {
  ...
  // ─── Init Fabric once campaign is loaded ─────────────────────
  useEffect(() => {
    if (!campaign || !canvasRef.current || fabricRef.current) return
    let alive = true

    const init = async () => {
      const { Canvas, Rect, Textbox, Image: FabricImage } = await import("fabric")
      ...
      // Zoom with wheel (Photoshop style — Ctrl+Scroll)
      ...
      if (savedLayers && Array.isArray(savedLayers) && savedLayers.length > 0) {
        ...
        for (const layer of sorted) {
          ...
          if (!asset) continue
          await addAssetToCanvas(fc, asset, layer, Rect, Textbox)
        }
      }
  ...
  async function addAssetToCanvas(fc: any, asset: Asset, layer: any, Rect: any, Textbox: any) {
    ...
    if (asset.type === "IMAGE") {
      // Se tiver imageUrl (base64 do PSD), carrega como imagem real
      if (asset.imageUrl) {
        try {
  ...
  async function addLayer() {
    ...
    await addAssetToCanvas(fc, asset, { posX: 100, posY: 100, width: Math.max(asset.type === "TEXT" ? 800 : 400, 200), scaleX: 1, scaleY: 1, rotation: 0 }, Rect, Textbox)
    fc.renderAll()
    doSave()
  }
      ...
      <div style={{ ...pS, right: 0, width: PW, borderLeft: "1px solid #2a2a2a", paddingTop: TH }}>
        ...
        {!selected ? (
          ...
          <div style={{ padding: 16, display: "flex", flexDirection: "column" as const, gap: 14 }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 5, marginTop: 8 }}>
                {SWATCHES.map(c => <div key={c} onClick={() => applyStyle("fill", c)} style={{ width: 24, height: 24, borderRadius: 4, background: c, cursor: "pointer", border: "2px solid #2a2a2a" }} />)}
              </div>
            </div>
            <div>
              <div style={secS}>Posição X / Y</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {[["X", "left"], ["Y", "top"]].map(([l, k]) => (
                  <div key={k}>
                    <label style={{ fontSize: 9, color: "#555", display: "block", marginBottom: 3 }}>{l}</label>
                    <input type="number" key={`${selected.id}-${k}`} defaultValue={Math.round(selected[k] ?? 0)}
                      onChange={e => { selected.set(k, +e.target.value); fabricRef.current?.renderAll(); doSave() }} style={inpS} />
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ padding: 16 }}>
            ...
            <div style={{ color: "#444", fontSize: 11, marginTop: 8 }}>Imagem selecionada.<br/>Mova e redimensione no canvas.</div>
          </div>
        )}
      </div>
